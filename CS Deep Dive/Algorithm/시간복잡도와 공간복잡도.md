...
데이터가 많아질수록 처리 시간이 로그(log)배 만큼 늘어난다. 예를들어 데이터가 10배가 되면 처리 시간은 약 20배가 된다. 대표적으로 병합 정렬, 퀵 정렬이 있다.
...
입력 크기가 `n`일때 연산 횟수가 $2^n$의 형태로 증가하면 시간복잡도는 $O(2^n)$ 이다. 대표적으로 피보나치 수열이 있다.
