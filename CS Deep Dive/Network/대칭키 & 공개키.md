...
### 대칭키 암호화 알고리즘
- **DES**
  - `64 bit의 블록`을 이용해 암호화 하며, `키 길이는 56 bit`이다.
  ...
  - 암호화 키(56bit)는 Round-key generator에 의해 각 라운드마다 서브키(48bit)를 생성하게 된다.
  - 이 서브키는 각 라운드에 라운드 키로 사용된다.  
...
- **AES**
  - `128 bit의 블록`을 이용해 암호화 하며, `키 길이는 128/192/256 bit`이다.
  - 고정된 라운드 수와 키 길이를 갖는 DES와 달리, AES는 `10/12/14개의 라운드 함수`와 대응하는 `키 길이 128/192/256 bit`를 갖는다.
  - 키의 길이는 달라도 라운드 키는 모두 128 bit이다.
  - DES 보다 안전하다.
  - 마지막 라운드를 제외한 각 라운드에서 4가지 연산(바이트 대체, 행 이동, 열 섞기, 라운드 키 더하기)을 이용해서 데이터 블록 전체를 처리한다.  
  <div style="display: flex; flex-direction: row; gap: 10px">
    <img src="https://postfiles.pstatic.net/MjAxOTExMThfMTk2/MDAxNTc0MDU0MTEzMzU0.HDnHyeJWnL6ccsEBIEyy560n095iG7rPW4bNsnD7XEYg.1OyEsUHA2G-eVTV7xquOe8eBWUzYD_X_kTv4-dkzn88g.JPEG.wnrjsxo/aes_structure.jpg?type=w773" alt="AES" height="500px">
    <img src="https://user-images.githubusercontent.com/102718303/208461812-5cabaaf5-6a67-4bf0-8a13-52861691aa38.png" alt="AES" height="500px" />
  </div>
...
- **RSA**
  - 암호화, 전자 서명에 사용된다.
  - 송신자가 수신자에게 공개키를 요청하면, 수신자는 자신의 공개키를 송신자에게 전송한다. 송신자는 수신자로부터 받은 수신자의 공개키를 이용해 평문을 암호화 한 후 수신자에게 전송한다. 수신자는 개인키를 이용해 헤당 암호문을 복호화한다.
