...
- **Encapsultation (캡슐화)**
  - 상위 계층에서 하위 계층으로 데이터를 전달하면서 각각의 층마다 데이터를 인식할 수 있도록 헤더에 정보를 추가하며, 이를 Encapsultation(캡슐화)라고 한다.
...
- **Decapsulation (역캡슐화)**
  - 하위 계층에서 상위 계층으로 데이터를 전달하면서 헤더를 제거하며, 이를 Decapsulation(역캡슐화) 라고 한다.
  - 응용 계층에 도달하면 원본 데이터만 남는다.

_\*PDU: 프로토콜 데이터 단위이며, OSI 모델의 정보 처리 단위이다._

<img weight="500px" alt="OSI" src="https://img1.daumcdn.net/thumb/R800x0/?scode=mtistory2&fname=https://t1.daumcdn.net/cfile/tistory/25303F355755856B02">
